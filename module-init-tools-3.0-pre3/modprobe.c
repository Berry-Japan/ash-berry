...
int main(int argc, char *argv[])
{
	...
	int ignore_proc = 0;
	enum noexists_response noexists = NOEXISTS_IGNORE;
	enum exists_response exists = EXISTS_IGNORE;
	...
	for (i = 0; i < num_modules; i++) {
		...
		if (remove)
			rmmod(&list, noexists, error, dry_run, verbose,
			      commands, ignore_commands, 0);
		else
			insmod(&list, NOFAIL(strdup(optstring)), newname,
			       exists, error, dry_run, verbose, modoptions,
